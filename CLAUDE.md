# CLAUDE.md

本文件指导 Claude Code 在本仓库工作。**`dut-postdoc` 是一个按 Karpathy「LLM Wiki」模式运转的个人研究知识库**——不是代码项目，而是一个由 Claude 增量构建与维护的、相互链接的 Markdown wiki。

参考方法论：Andrej Karpathy, *LLM Wiki: A Personal Knowledge Base Pattern*（<https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f>）。

## 它是什么

一句话：**在「原始资料」和「我」之间，维护一个持久、结构化、可被 LLM 读写的中间层**，这样每次提问不必从零重读论文，而是查询一个不断沉淀的 wiki。

> 维护知识库里真正烦人的不是阅读和思考，而是记账（bookkeeping）。LLM 恰好擅长这种人类会半途放弃的维护工作。**人的职责是：挑选资料、指明分析方向、提出好问题；其余交给 Claude。**

## 三层架构

| 层 | 内容 | 谁拥有 | 规则 |
|---|---|---|---|
| **原始源层** | 论文 PDF、文章、图片等不可变资料 | 人 | Claude 只读，**永不修改**；是最终事实来源。PDF 本地存放，不入版本控制（见 `.gitignore`） |
| **Wiki 层** | 文献笔记、调研、概念页、实体页、论文草稿 | Claude | Claude 全权创建/维护页面与交叉引用 |
| **Schema 层** | 本文件（CLAUDE.md）+ 各模板 | 人定，Claude 遵守 | 约定、工作流、目录结构 |

## 目录地图

```
dut-postdoc/
├── CLAUDE.md          # 本文件：Schema 层
├── index.md           # 根总目录：全库内容地图（每次 ingest 后更新）
├── log.md             # 时间线：append-only，记录每次 ingest/query/lint
├── README.md          # 给人看的仓库说明
│
├── literature/        # 文献笔记（单篇论文级 ≈ summary 页）
│   ├── _index.md      # 分区索引
│   ├── topology-opt/  # 拓扑优化
│   ├── fem/           # 有限元方法
│   └── others/
├── research/          # 调研/综合（课题/方向级 ≈ synthesis 页）
│   └── _index.md
├── concepts/          # 概念页（跨源综合：一个概念一页，如 PIML、MMC/MMV）
│   └── _index.md
├── entities/          # 实体页（人/团队/机构/方法/软件，如 郭旭团队）
│   └── _index.md
├── papers/            # 自己写的论文草稿
├── talks/             # 报告/讲稿（LaTeX）
└── assets/
    ├── refs.bib       # 共用参考文献库
    └── templates/     # 各类页面模板
```

页面类型速记：
- **summary（文献笔记）**：一篇论文一页，落在 `literature/<方向>/`。
- **synthesis（调研）**：一个课题跨多篇论文，落在 `research/`。
- **concept（概念页）**：一个反复出现的概念跨多个来源的提炼，落在 `concepts/`。
- **entity（实体页）**：一个人/团队/机构/方法/软件的档案卡，落在 `entities/`。

## 写作约定

- **语言**：全中文（专有名词、方法名、变量保留英文）。
- **文件名**：英文 kebab-case；文献笔记沿用 `<作者><年份>-<关键词>` 如 `Huang2022-PIML-universal.md`。
- **Frontmatter**：每个 wiki 页面顶部带 YAML（按对应模板）。`status`、`tags`、日期字段必填。
- **双链**：页面间一律用 Obsidian `[[wikilink]]`，**链接要给足**——宁可多链。链一个尚不存在的页面也可以，它标记「将来要补的页」。
- **引用要可溯源**：综合性结论尽量标注来源页（`[[...]]`）或 `refs.bib` 的 cite key，不凭空断言。
- **不编造**：拿不准的事实标注「待确认」，绝不替我虚构数据、结论或文献。

## 三个核心操作

### 1. Ingest（吸收新资料）

当我给一篇新论文/文章/图片时：

1. **读** 原始资料（PDF/图片/链接）。
2. **讨论** 和我过一遍核心要点（一句话概括 + 它解决什么 + 对我有什么用）。
3. **写 summary**：用 `assets/templates/literature-note.md` 在对应 `literature/<方向>/` 建文献笔记；图片存同级 `assets/`。
4. **更新 `refs.bib`**：补上该文献条目。
5. **横向刷新**：更新受影响的 **概念页**（`concepts/`）、**实体页**（`entities/`，如涉及某团队/方法）、相关 **调研**（`research/`）——一次 ingest 可能要动 5–15 个文件。
6. **更新索引**：`literature/_index.md`、根 `index.md`。
7. **记一笔 log**（见下）。

### 2. Query（查询）

我提问时：

1. 在 wiki 内**搜索**相关页（concepts → entities → research → literature 顺序通常最高效）。
2. **综合**作答，**带引用**（标注来源页 `[[...]]`）。
3. 如果这条问答有长期价值，**回填**成一个永久页面（新概念页，或补进已有页），并更新 `index.md`。

### 3. Lint（健康检查）

我说「lint / 体检 / 整理」时，扫描并报告（不擅自大改，先列清单）：

- **矛盾**：不同页面对同一事实的冲突说法。
- **过期**：与新资料冲突的旧结论。
- **孤页**：没有任何页面链入的页面。
- **缺链**：本该互链却没链的页面、指向不存在页面的死链。
- **空缺**：`_index.md`/`index.md` 漏登记的页面；frontmatter 缺字段。
- 在 `log.md` 记一笔 lint 结果。

## index.md 与 log.md

- **`index.md`（根总目录）**：面向内容的全库地图，按类别列出各页 + 一句话摘要 + 关键元数据。**每次 ingest 后更新。** 分区细目仍由各 `_index.md` 维护，根 `index.md` 汇总并指向它们。
- **`log.md`（时间线）**：append-only，每次操作追加一条可解析记录：
  ```
  ## [YYYY-MM-DD] <ingest|query|lint|edit> | <简述>
  - 动了哪些文件 / 关键结论
  ```
  只增不改历史条目。

## 安全与隐私

- 原始 PDF 不入版本控制（已在 `.gitignore`）。
- 这是我个人的研究积累；涉及未发表想法、团队内部信息时谨慎措辞，不外传、不联网检索敏感人名/单位细节。
