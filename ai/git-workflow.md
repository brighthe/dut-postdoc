...
- 对同时含“会话前改动 + 本次改动”的文件，先 `git diff` 确认内容再决定是否纳入。若某文件既含无关改动又含本次改动，不得整文件暂存：先备份，临时移除无关段落后 `git add`，再从备份还原工作树，使暂存区只含本次内容。
- **目录移动或重组时，文件清单必须用 `git ls-files <路径>` 生成，不能只按 `*.md` 搜索**。本仓库跟踪 `.tex`、`.py`、`.pdf`、`.gitkeep` 等非 Markdown 文件，它们通常没有任何 wikilink 指向，链接检查照不到；2026-07-30 的 `research/` 重组即因此漏掉 `postdoc-research-plan.tex`，需另起一次提交补救。
