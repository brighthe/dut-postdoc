...
- **Wiki 层**（文献译文、调研、工作汇报、概念页、实体页、论文草稿、事件档案）：AI 增量创建、维护与互链。
- **Schema 层**（`ai/` 下的规则与 `ai/templates/` 模板 + 根目录工具入口 `CLAUDE.md`／`AGENTS.md` + `.gitignore`）：人定，AI 遵守。根目录只放工具强制查找的入口桩，规则本体一律在 `ai/`；`.gitignore` 是存储归属规则的执行体。
- 存储归属（iCloud／Zotero／Git 的分工、`sources/` 本地缓存、`source:` 逻辑标识解析、图件目录）以 [git-workflow.md](git-workflow.md)「原始资料与派生文件的存储归属」为唯一权威，本文件不复述。

## 写作约定

- **语言**：全中文（专有名词、方法名、变量保留英文）。
- **文件名与 Citation Key**：文献页面用 `AuthorYear-short-topic` basename（如 `Huang2022-problemindependentmachine.md`），中文译文同 basename 加 `-zh`；Citation Key 与文件名分离，只存 frontmatter 的 `citekey` 和 `literature/refs.bib`。
...
- **链接路径**：一律用相对于当前文件的路径（同目录写文件名，跨目录用 `../`），不用 vault 根路径或跨目录裸文件名。移动或重命名页面时必须同步改写该页全部出链和指向它的全部入链，并复核每条链接可按相对路径解析；解析失败时不回退为按文件名匹配。例外：所属方向 `assets/` 下的图件用裸文件名嵌入（`![[xx.png]]`），图件的目录归属见 [git-workflow.md](git-workflow.md)「图件归属」，嵌入与图注口径见 [paper-translation-workflow.md](paper-translation-workflow.md) §3.3。
...
- **新建或升级页面**：中文译文、主题索引、汇报页或事件归档前，读 [page-schemas.md](page-schemas.md)。
...
- **论文提炼与撰写**：当任务涉及 `papers/` 下草稿重构、变分推导或章节撰写时，**禁止直接长篇输出或擅自套用流程**。默认必须先用一句话向用户提议（如：“*建议调用 `paper-drafting` 技能（严格对照博士论文第五章与 CICP 投稿标准），请问是否同意加载？*”）；仅在用户明确同意或显式指示时，才读取并执行 [paper-drafting-workflow.md](paper-drafting-workflow.md)。
