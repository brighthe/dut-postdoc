...
> 本页连接 Hu–Zhang 混合有限元（Hu–Zhang Mixed Finite Element Method）的理论基础、程序实现、实测收敛证据与拓扑优化论文进展。核心方法指基于对称应力和位移为独立未知量的 $H(\mathrm{div})$ 协调混合有限元方法；在低阶 $k=1,2$ 时配套位移跳量稳定化，在复杂边界角点配套切向顶点松弛。**命名边界**：本主题指上述混合元方法本身及其在拓扑优化中的应用，不代表任意「混合有限元」的通用理论；当前研究产出以论文主线推进，未设置独立的 Hu–Zhang 项目分支。

## 稳定知识

| 页面 | 一句话 | 状态 |
|---|---|---|
| [[huzhang-mixed-fem]] | Hellinger–Reissner 变分原理、鞍点代数结构、非齐次牵引提升、跳量稳定化缩放律与角点部分顶点松弛理论 | in-progress |
| [[../linear-elasticity]] | 位移型线弹性基础、强/弱形式与古典位移元对比（本页的出发问题） | in-progress |
| [[../external-loads]] | 外载荷数据的正则性、集中力适定性与两套变分形式下的施加格式（本页牵引提升的通用背景） | in-progress |
...
> 本主题当前没有独立项目分支，研究产出以下列论文主线推进；这是本页与其他主题入口在该节命名上的差异。
...
- 关联主题：[[../external-loads]] — 外载荷的对偶空间、适定性与离散施加格式。
...
- 通用外载荷数学（各类外载荷数据的正则性、集中力在 $H^{-1}$ 与 $H^{-1/2}$ 下的非适定性、一致节点力系数与 $P_1$ 迹 $L^2$ 投影的守恒性）由 [[../external-loads]] 维护；本目录只保留胡张形式下边界条件的对偶语义与牵引提升构造。
