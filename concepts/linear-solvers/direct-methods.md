...
MUMPS、PARDISO、CPardiso、SuperLU / SuperLU_DIST 是有限元软件中常见的稀疏直接求解器**实现**。它们通常不由应用代码直接调用，而是经框架的求解器接口接入：PETSc 的 `PCLU` / `PCCHOLESKY` 就是这样一格接口，底层实现由 `MatSolverType` 选定，PETSc 自带的分解只支持顺序矩阵，并行矩阵必须转给外部包。本节比较的是实现层，不是接口层。

这些实现在算法上同属分解—回代框架，差别集中在**并行模型**上，而并行模型决定了可达的问题规模上限与接口形态。
...
分布式内存模型买到的是**容量上限**：因子切分到各进程后可跨节点累加内存，规模不再受单节点内存限制。代价是通信开销与切分带来的冗余，因此在同一节点内、相同核数下，把因子切碎的多进程配置不一定快于单进程多线程配置。孰优孰劣取决于实现的并行粒度与通信模式，也随算例规模与底层 BLAS 后端变化，须按实现实测确定，没有通用结论。三层并行的一般语义见 [[../gpu-hpc/parallel-levels]]。
