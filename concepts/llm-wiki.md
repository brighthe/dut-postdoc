...
- **Schema 层**：`ai/common/`、各工具入口、模板、目录约定和日志格式。人制定规则，AI 工具遵守规则执行。
...
`dut-postdoc` 的目标不是收藏资料，而是支撑博士后阶段的研究判断与写作：围绕 [[piml]]、拓扑优化、有限元方法、矩阵无关求解、高性能计算等方向，把论文精读、技术路线、团队谱系、开放问题和论文草稿放进同一套可追溯结构里。具体工具可以是 Codex、Claude Code 或后续其他 AI 助手；共同遵守的知识库规则放在 `ai/common/llm-wiki-workflow.md`。
