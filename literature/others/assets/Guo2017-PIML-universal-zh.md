...
$$
值得注意的是，尽管本文在 EMsFEM 分析中仅采用了四节点粗分辨率单元和线性边界条件，但实际上可以引入更多节点以及更复杂的边界条件，以提高 EMsFEM 的精度。

# 4 基于机器学习的 EMsFEM 形函数构造

上一节已经描述了 EMsFEM 形函数的计算方法。然而，值得注意的是，当 EMsFEM 应用于拓扑优化时，由于每一个粗分辨率单元中的材料分布在迭代过程中不断变化，因此必须在每一步迭代中重新构造形函数。在这种情况下，即使采用 EMsFEM 进行结构分析，也不能期望有限元分析时间得到显著降低。

为了绕开上述困难，本文提出了一种新的处理流程，即用一个离线训练好的机器学习模型，替代耗时的在线形函数构造过程。在这种处理方式下，机器学习模型的输入参数仅为某一个特定粗分辨率单元内部的细分辨率单元密度；模型的输出则是该粗分辨率单元内 $N_{ixx}^l$、$N_{ixy}^l$、$N_{iyx}^l$ 和 $N_{iyy}^l$ 的节点值，其中 $i=1,\ldots,4$，$l=1,\ldots,N$。由这些预测得到的形函数节点值，可以非常快速地生成粗分辨率单元的刚度矩阵，并将其直接用于 EMsFEM 分析。
...
（2）该方法在理论上也是局部决定的。这是因为所学习的形函数仅由所考虑粗分辨率单元内部的材料分布决定，而与其他任何粗分辨率单元中的材料分布无关。
（3）不同于大多数已有工作，本文所提出机器学习模型的训练过程不依赖任何特定的拓扑优化问题，而只需随机生成局部材料密度即可完成。因此，训练得到的形函数可以用于求解由相同偏微分方程控制的任意类型拓扑优化问题。

# 5 拓扑优化求解流程

与传统 SIMP 方法的求解流程相比，本文方法唯一的区别在于：采用机器学习增强的 EMsFEM 来计算细分辨率网格的节点位移。本文方法的高求解效率主要来源于以下两个方面：
...
本节通过求解短悬臂梁算例、L 型梁算例以及 MBB 梁算例，验证本文所提出方法的有效性与精度。所有数值算例中涉及的参数均假定为无量纲量。实体材料与弱材料的杨氏模量分别取为
$$
...
与经典 SIMP 方法对应的结果相比，$t_{\mathrm{it}}$ 分别降低了 $75.77\%$（对于 $10\times 10$ 情形）和 $74.52\%$（对于 $m=5\times 5$ 情形）。在本文提出的 PIML 增强方法中，每次迭代中 OC 更新所消耗的时间占据主要部分。在经典 SIMP 方法中，总计算时间的 $80.82\%$ 被用于有限元分析；而在本文提出的方法中，OC 更新所消耗的时间超过了总计算时间的 $77\%$。实际上，对于该问题，本文方法已经使有限元分析时间减少了近 $10$ 倍。
