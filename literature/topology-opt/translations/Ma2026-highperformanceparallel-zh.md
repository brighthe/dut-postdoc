...
<strong>图 1</strong>　PIML 模型中 <em>m</em> = 5 的三维子结构示意图。
...
<strong>图 2</strong>　PIML 增强的子结构分析算法示意图。
...
<strong>图 3</strong>　PIML 增强型大规模拓扑优化的串行和并行算法流程图。
...
<div align="center" id="fig-4">

<strong>图 4</strong>　<em>m</em> = 10 时并行 PIML 增强求解策略中的网格分解示意图（一个包含 10.24 亿个单元的三维设计域被分布到 2000 个 CPU 核心中）。
...
<div align="center" id="fig-5">

<strong>图 5</strong>　悬臂梁算例的设计域和边界条件。
...
<div align="center" id="fig-6">

<strong>图 6</strong>　使用该方法获得的优化悬臂梁：（a）PDE 滤波器半径为 0.042，结构柔顺度为 377.0，共 3456 万个单元；（b）PDE 滤波器半径为 0.0125，结构柔顺度为 270.99，共 1.28 亿个单元。
...
<div align="center" id="fig-7">

<strong>图 7</strong>　MBB 梁算例的设计域和边界条件。

</div>

![[Pasted image 20260607091741.png]]

<div align="center" id="fig-8">

<strong>图 8</strong>　使用该方法获得的优化 MBB 梁：（a）150 万个单元，密度滤波器半径为 0.05，每次迭代 2.6 s，结构柔顺度为 218.1；（b）150 万个单元，密度滤波器半径为 0.07，每次迭代 2.8 s，结构柔顺度为 221.9。
...
<div align="center" id="fig-9">

<strong>图 9</strong>　10 亿单元悬臂梁算例中，串行算法以及使用 2000 和 8192 个 CPU 核心的并行 PIML 增强算法的计算成本。
...
<div align="center" id="fig-10">

<strong>图 10</strong>　通过并行 PIML 增强算法获得的 10 亿单元悬臂梁优化设计。
...
<div align="center" id="fig-11">

<strong>图 11</strong>　通过并行 PIML 增强算法获得的百亿自由度悬臂梁优化设计。

</div>

![[Pasted image 20260607132052.png]]

<div align="center" id="fig-12">

<strong>图 12</strong>　百亿自由度优化悬臂梁的（a）1/4 和 1/2 位置切片视图及（b）局部结构细节。
...
<div align="center" id="fig-13">

<strong>图 13</strong>　百亿自由度悬臂梁算例的收敛历史：（a）目标函数值和体积约束；（b）每次迭代的总时间及计算全局缩聚刚度矩阵的时间。
...
<div align="center" id="fig-14">

<strong>图 14</strong>　类桥梁算例的设计域和边界条件。

</div>

![[Pasted image 20260609110639.png]]

<div align="center" id="fig-15">

<strong>图 15</strong>　通过并行 PIML 增强算法获得的 33.2 亿单元类桥梁算例优化设计。
...
<div align="center" id="table-1">

<strong>表 1</strong>　使用悬臂梁算例测试弱扩展效率。
...
<div align="center" id="table-2">

<strong>表 2</strong>　不同多重网格层级和最大进程利用率下 PIML 增强拓扑优化算法的时间成本。
...
<strong>表 3</strong>　使用 819 万单元悬臂梁算例测试强扩展加速比。
...
<div align="center" id="fig-16">

<strong>图 16</strong>　不同尺度下并行 PIML 增强算法的精度、效率和优化结构性能。
...
<div align="center" id="table-4">

<strong>表 4</strong>　不同问题规模下实现的最快计算速度。
