...
- **撤回一处过度断言**：§3 末段原称「在同一节点内、相同核数下，把因子切碎的多进程配置**通常慢于**单进程多线程配置」。该表述被实测推翻——同节点同核数下，两种实现的表现方向相反，其中一种在多进程布局下反而更快。改为「不一定快于」，并补充孰优孰劣取决于实现的并行粒度与通信模式，也随算例规模与 BLAS 后端变化，须实测确定。按 `_index.md` 管理边界，不把具体数值写入概念页。
- 两处错误均由同日上一条引入，该条已提交推送，故另起本条更正，不改历史条目。
