...
作为基础柔顺度最小化算例，本节考察经典的中点受载两端固支梁结构，如图 4.1 所示。设计域 $\Omega$ 为长宽比 $8:1$ 的矩形薄板，几何尺寸设定为 $L \times 0.125L$，其中特征长度取 $L=160\,\mathrm{mm}$（即 $160\,\mathrm{mm} \times 20\,\mathrm{mm}$）。设计域左、右两侧垂直边界施加完全固支约束（位移 $\boldsymbol{u} = \boldsymbol{0}$），在下边界中点 $(0.5L, 0)$ 处施加竖直向下的集中载荷 $P = 3\,\mathrm{N}$。在数值实现中，依据第 2.2 节的牵引提升策略，将其转化为特征长度 $l = 1\,\mathrm{mm}$ 上的等效局部均布牵引力；其余边界均为自由应力边界（$\boldsymbol{\sigma}\boldsymbol{n} = \mathbf{0}$）。考虑到几何构型及物理响应的对称性，计算中取左半部分设计域（$80\,\mathrm{mm} \times 20\,\mathrm{mm}$）进行离散求解，并在对称面处施加相应的位移与应力约束。采用平面应力假设，实体材料参数取为 Young 模量 $E_0 = 30\,\mathrm{MPa}$，Poisson 比 $\nu_0 = 0.4$。优化目标为在体积分数约束 $\bar V = 0.4$ 下最小化结构柔顺度，密度过滤半径固定为 $r_{\min} = 2.4\,\mathrm{mm}$。

<!-- 投稿矢量源：figures/fig4_1_schematic.eps；PDF 预览：figures/fig4_1_schematic.pdf；TikZ 源：figures/fig4_1_schematic.tex。 -->
![两端固支梁完整设计域与左半对称设计域示意图](figures/fig4_1_schematic-v9.png)
**图 4.1**  两端固支梁完整设计域（左）与左半对称设计域（右）的载荷与边界条件示意图。完整域关于竖直中线 $x=0.5L$ 对称，计算中取左半域并在对称面施加 $u_x=0$ 与 $\sigma_{xy}=0$ 的对称约束。

在离散阶次选择上，本算例对比测试了 $k=2, 3, 4$ 阶离散格式。需特别说明的是，拓扑优化中未采用 $k=1$（即 $P_1$ 应力 / $P_0$ 位移）的最低阶配置：尽管该配置在引入跳量稳定化后可实现静力前向求解，但其 $P_0$ 位移空间无法完备包含刚体位移（Rigid Body Motion, RM）空间，即丧失了表征单元局部微小旋转的能力，导致低密度区局部应变能评估严重失真，极易引发非物理拓扑震荡。因此，选取低阶跳量稳定化 $k=2$（其 $P_1$ 位移已完备包含 RM 空间）作为验证低阶格式有效性的下限，而选取 $k=3, 4$ 用于考察高阶原生格式的数值性能。求解域统一采用 $80 \times 20$ 的结构化交叉三角形网格剖分。

不同离散方法及阶次下的最终拓扑构型对比见图 4.2，优化过程的收敛历史曲线见图 4.3。数值实验结果表明：

1. **拓扑构型高度一致**：标准位移有限元（LFEM）与 Hu–Zhang 混合有限元（HZMFEM）在相同多项式阶次 $k=2,3,4$ 下收敛得到的最终拓扑构型（图 4.2）展现出极高的物理吻合度（单元级二值化一致率 $98.8\%$），均清晰、准确地捕捉到了两端固支梁经典的交叉承载杆件（Cross-braced truss）构型，验证了混合元框架驱动材料演化的正确性；
2. **柔顺度评估与离散一致性**：在定量指标方面，标准位移法在 $k=2,3,4$ 阶次下的最终柔顺度分别收敛于 $C \approx 31.94,\,32.05,\,32.07$，Hu–Zhang 混合元相应收敛于 $C \approx 33.07,\,32.58,\,32.32$。两种方法在同一网格与阶次下的目标评估差异不超过 $3.5\%$，且随阶次升高迅速缩小（$k=4$ 时约 $0.8\%$）。该一致性源于两者从不同变分原理出发评估同一真实解：位移法最小化势能，严格给出柔顺度的下界近似；Hu–Zhang 混合元基于 $H(\operatorname{div})$ 协调的对称张量空间由互补能变分原理求解，其互补能评估实测略高于位移法的势能评估。两者的评估值均随多项式阶次提高而收敛趋同，验证了目标评估的离散收敛性。因此 Hu–Zhang 混合元在柔顺度优化中的优势主要体现在应力场逼近精度（第 4.1 节，$\mathcal{O}(h^{k+1})$ 超收敛）与应力约束评估的可靠性（第 4.2.3 节），而非柔顺度数值的系统性偏移；
...
![标准位移法收敛曲线](figures/fig4_3a_lfem_conv.png)
![Hu-Zhang混合法收敛曲线](figures/fig4_3b_hzmfem_conv.png)
**图 4.3**  两端固支梁优化过程的目标函数（柔顺度）与体积分数收敛历史曲线对比（(a) 标准位移法；(b) Hu–Zhang 混合法）

#### 4.2.2 近不可压缩拓扑优化

为检验任意次 Hu–Zhang 混合有限元方法在逼近不可压缩极限（$\nu_0 \to 0.5$）下的抗体积自锁（Locking-free）能力与数值鲁棒性，本小节采用经典的二维轴承装置（Bearing Device）拓扑优化算例进行深入对比分析。

**算例模型与边界条件设置**：设计域为长宽比 $3:1$ 的矩形区域 $\Omega = [0, 3L] \times [0, L]$，其中特征长度取 $L = 40\,\mathrm{mm}$。边界条件与受力示意如图 4.4 所示：底边施加全固支位移约束（$u_x = u_y = 0$）；顶边施加竖直向下的均布牵引载荷 $\boldsymbol{t} = (0, -t_0)^{\top}$，其中载荷幅值 $t_0 = 8 \times 10^{-2}\,\mathrm{N/mm}$；左右两侧为自由边界。材料基础杨氏模量设定为 $E_0 = 1.0\,\mathrm{MPa}$，二维弹性采用**平面应变（Plane Strain，$\varepsilon_{zz}=0$）**本构假设，以严格施加面内无散度体积变形约束 $\operatorname{div}\boldsymbol{u} = \varepsilon_{xx} + \varepsilon_{yy} \approx 0$。优化目标为最小化结构总体柔顺度，体积分数上限约束为 $\bar{V} = 0.35$。求解域采用 $120 \times 40$ 的结构化交叉三角形网格剖分，密度过滤半径取 $r_{\min} = 2.0\,\mathrm{mm}$，初始设计变量统一取均匀分布 $\rho_0 = 0.35$。

![二维轴承装置设计域、载荷与边界条件示意图](figures/fig4_4_bearing_schematic.png)
**图 4.4**  二维轴承装置设计域、载荷与边界条件示意图

在近不可压缩优化中，为严格隔离数值离散误差并排除材料插值引起的非物理干扰，采用修正的泊松比插值模型（式 (3.25)）：
$$
\nu(\rho) = \nu_{\mathrm{void}} + \rho^{p_\nu} (\nu_0 - \nu_{\mathrm{void}}),
$$
式中取弱材料泊松比 $\nu_{\mathrm{void}} = 0.3$，惩罚指数取 $p_\nu = 1$。该设置保证了低密度孔洞区呈现良好的可压缩性，从而彻底消除了因弱材料剪切模量与体积模量严重失衡所导致的非物理虚假静水压力支撑，使得优化构型的差异完全取决于底层有限元离散在处理不可压缩物理约束时的数值稳定性。实验分别设置可压缩基准组（$\nu_0 = 0.3$）与近不可压缩实验组（$\nu_0 = 0.4999$）。

![不同离散方法在可压缩与近不可压缩工况下的最终拓扑构型对比](figures/fig4_5_bearing_k2_topologies.png)
**图 4.5**  $k=2$ 阶次下标准位移法与 Hu–Zhang 混合法在不同泊松比下的最终拓扑构型与柔顺度对比（上排：标准位移法 LFEM；下排：Hu–Zhang 混合法 HZMFEM；左列：可压缩基准组 $\nu_0 = 0.3$；右列：近不可压缩实验组 $\nu_0 = 0.4999$）

**$k=2$ 阶次下 LFEM 与 HZMFEM 对比分析**：
图 4.5 展示了多项式阶次 $k=2$ 下，标准位移法（LFEM）与引入跳量稳定化的 Hu–Zhang 混合法（HZMFEM）在两种泊松比工况下的最终拓扑构型与目标柔顺度 $C$，相关定量指标汇总于表 4.2。数值对比结果揭示了深刻的力学与离散机理：

1. **可压缩基准组（$\nu_0 = 0.3$）的一致性**：在常规可压缩状态下，标准二次位移法（图 4.5(a)，$C \approx 123.38$，196 步收敛）与 Hu–Zhang 混合法（图 4.5(c)，$C \approx 128.45$，283 步收敛）均演化出边界清晰、传力高效的多拱形支撑结构（Multi-arch support structure）。两者构型高度吻合，柔顺度数值差异在合理范围内（约 $4.1\%$），验证了混合变分框架驱动材料演化的正确性。
2. **近不可压缩极限（$\nu_0 = 0.4999$）下的性能分化与体积自锁**：当材料逼近不可压缩极限时，两类离散格式的表现产生质的差异：
   - **标准位移法（LFEM）遭遇严重体积自锁**：在平面应变条件下，$\nu_0 \to 0.5$ 导致拉梅第一常数 $\lambda = \frac{E\nu}{(1+\nu)(1-2\nu)} \to \infty$，强加了苛刻的面内无散度变形约束（$\operatorname{div}\boldsymbol{u} \approx 0$）。低阶位移多项式空间因自由度不足以满足单元逐点无散度条件，引发严重的体积自锁。为在有限元方程中强行释放虚假静水压力，拓扑优化算法被迫在结构内部大面积堆积材料并形成病态的虚假连杆与铰链连接（图 4.5(b)），彻底破坏了原本合理的拱形承载拓扑；同时，其计算所得的柔顺度异常骤降至 $C \approx 55.79$（由于虚假铰链处材料反复震荡，达到最大 500 步退出）。这种数值上的“超强刚度”完全是由体积自锁导致的人工刚化假象（Artificial Stiffening），严重背离了实际力学规律。
   - **Hu–Zhang 混合法（HZMFEM）天然免疫体积自锁**：基于 Hellinger–Reissner 原理的混合格式将对称应力作为独立主变量。当 $\lambda \to \infty$ 时，柔度算子在全应力空间上的作用退化为对偏应力分量的控制：$(\mathcal{A}\boldsymbol{\sigma}, \boldsymbol{\sigma}) \to \frac{1}{2\mu}\|\operatorname{dev}\boldsymbol{\sigma}\|_{0,\Omega}^2$。由于柔度双线性型在不可压缩极限下依然保持有界，且与位移场的耦合满足离散 inf-sup 稳定性，混合鞍点系统在 $\nu_0 \to 0.5$ 依然具备极优的适定性。如图 4.5(d) 所示，Hu–Zhang 混合元依然稳健地生成了清晰、平滑且无铰链缺陷的多拱形构型，其柔顺度在第 297 步平滑收敛于 $C \approx 102.82$，客观、真实地反映了近不可压缩材料真实的抗剪切变形承载能力。

**表 4.2**  二维轴承装置在不同离散方法及泊松比 $\nu_0$ 下的拓扑优化定量结果对比（$k=2$）
...
综上所述，本算例充分证实了 Hu–Zhang 混合有限元在近不可压缩材料拓扑设计中相对于传统位移法的压倒性数值优势，为解决橡胶弹性体及不可压缩生物多孔结构的优化设计提供了坚实、可靠的离散计算基础。


#### 4.2.3 局部应力约束拓扑优化

为进一步验证 Hu–Zhang 混合有限元在直接处理高精度应力约束拓扑优化中的独特优势，本小节引入经典的端部中点受载二维悬臂梁（Cantilever Beam）局部应力约束拓扑优化问题。

**算例模型与边界条件设置**：设计域为长宽比 $2:1$ 的矩形区域 $\Omega = [0, 2L] \times [0, L]$，其中特征长度取 $L = 40\,\mathrm{mm}$（总几何尺寸为 $80\,\mathrm{mm} \times 40\,\mathrm{mm}$）。边界条件与受力示意如图 4.7 所示：左侧边界施加全固支位移约束（$x=0, \boldsymbol{u} = \boldsymbol{0}$）；右侧边界中点 $(2L, 0.5L)$ 附近的局部对称区间 $[17, 23]\,\mathrm{mm}$（载荷特征宽度 $l = 6.0\,\mathrm{mm}$）施加向下均布外力，合力大小为 $P = 400\,\mathrm{N}$（均布牵引强度为 $\bar{t}_l = 66.67\,\mathrm{N/mm}$）；上下两侧为自由边界。材料基础参数设定为杨氏模量 $E_0 = 70\,000\,\mathrm{MPa}$、泊松比 $\nu_0 = 0.25$，允许 von Mises 等效应力上限设定为 $\bar{\sigma} = 180\,\mathrm{MPa}$。

![二维悬臂梁设计域、载荷与局部应力约束示意图](figures/fig4_7_cantilever_schematic.png)
**图 4.7**  二维悬臂梁设计域、载荷与局部应力约束示意图

**优化问题数学建模与增广拉格朗日求解框架**：
优化目标为最小化结构的总体积（材料用量），同时在全设计域的每一个单元（或积分点）上严格强加局部 von Mises 等效应力约束：
$$
\begin{aligned}
\min_{\boldsymbol{\rho}} \quad & f_V(\boldsymbol{\rho}) = \frac{1}{|\Omega|} \sum_{e=1}^{N_e} v_e \widetilde{\rho}_e \\
\text{s.t.} \quad & g_e(\boldsymbol{\rho}, \boldsymbol{\sigma}_h) = \frac{\sigma_{\mathrm{vm}}(\boldsymbol{\sigma}_{h,e})}{\bar{\sigma}} - \eta(\widetilde{\rho}_e) \le 0, \quad \forall\,e \in \{1, 2, \dots, N_e\}, \\
& \text{线弹性混合状态方程 } (2.8), \\
& 0 \le \rho_e \le 1, \quad \forall\,e \in \{1, 2, \dots, N_e\},
\end{aligned}
$$
式中 $v_e$ 为单元面积，$\widetilde{\rho}_e$ 为密度过滤后的物理密度，$\sigma_{\mathrm{vm}}(\boldsymbol{\sigma}_{h,e})$ 为单元 $e$ 中心（或积分点）处由当前有限元解求得的 von Mises 应力。为彻底避免低密度孔洞区（$\widetilde{\rho}_e \to 0$）因应力分母趋零引发的“奇异应力（Stress Singularity）”病态现象，采用无密度分母的表观应力松弛模型（式 (3.29)）：
$$
\eta(\widetilde{\rho}_e) = \widetilde{\rho}_e^p + \epsilon (1 - \widetilde{\rho}_e^p),
$$
式中取应力松弛参数 $\epsilon = 10^{-4}$，SIMP 惩罚因子取 $p = 3$。该松弛策略在实体材料区（$\widetilde{\rho}_e = 1$）严格恢复真实物理应力约束 $\sigma_{\mathrm{vm}} \le \bar{\sigma}$，而在孔洞区（$\widetilde{\rho}_e = 0$）将等效约束放宽至 $\sigma_{\mathrm{vm}} \le \epsilon \bar{\sigma}$，从而确保了优化全过程约束函数与灵敏度导数的连续性与数值稳定性。

针对包含海量局部约束（约束数目等于网格单元数 $N_e$）的非线性规划问题，采用可分离增广拉格朗日法（Augmented Lagrangian Method, ALM）构建无约束子问题，外循环自适应更新各单元的拉格朗日乘子向量 $\boldsymbol{\lambda}$ 与罚因子 $\mu$，内循环调用移动渐近线算法（MMA）更新设计变量。

**离散工况与对比分析设置**：
为全面评估 Hu–Zhang 混合有限元应力约束拓扑优化框架的有效性，在统一参数设置（$80\times 40$ 结构化交叉三角形网格剖分，密度过滤半径 $r_{\min} = 6.0\,\mathrm{mm}$，均匀初始密度分布 $\rho_0 = 0.5$）下设置严格的对照实验。分别采用多项式阶次均为 $k=2$ 的标准位移法（LFEM $k=2$）与 Hu–Zhang 混合法（HZMFEM $k=2$）进行独立求解与对比。

**优化结果对比与力学机理讨论**：
图 4.8 给出了标准位移法（LFEM $k=2$）与 Hu–Zhang 混合法（HZMFEM $k=2$）优化得到的最终拓扑构型与归一化 von Mises 等效应力分布对比。定量指标汇总于表 4.3。

![位移法与 Hu–Zhang 混合法在局部应力约束下的最终拓扑构型与应力分布对比](figures/fig4_8_cantilever_stress_topologies.png)
**图 4.8**  二维悬臂梁局部应力约束拓扑优化结果对比：(a) 标准位移法（$k=2$）最终拓扑构型；(b) 标准位移法（$k=2$）归一化 von Mises 应力分布；(c) 胡张混合法（$k=2$）最终拓扑构型；(d) 胡张混合法（$k=2$）归一化 von Mises 应力分布

**表 4.3**  悬臂梁局部应力约束拓扑优化对比结果汇总表（SOPTX 独立复现计算结果）
...
从图 4.8 与表 4.3 的对比中可以清晰地得出以下结论：
1. **宏观拓扑构型的一致性**：无论是标准位移法还是 Hu–Zhang 混合有限元法，均成功演化出边界清晰、传力路径明确的经典双跨 Warren 桁架交叉承载结构（包含两道外侧受拉/压主弦杆与两道内侧斜撑杆），两者在宏观拓扑形态上表现出高度的一致性；
2. **局部应力场连续性与保真度的本质差异**：深入观察应力分布云图（图 4.8(b) 与 (d)）可以发现，传统位移法因对应力场进行形函数求导后处理，导致应力在单元交界处存在明显的非物理跳跃与锯齿状伪集中；而 Hu–Zhang 混合有限元以对称应力张量为原生基本变量，基于底层 $H(\mathrm{div})$ 空间的强协调性，天然保证了跨单元边界法向应力分量的严格连续，实体杆件内部与交叉节点处的应力场表现得更加平滑、圆顺，彻底消除了网格依赖性带来的应力数值震荡；
3. **优化精度与物理安全性评析**：在定量指标上，位移法最终体积分数为 $V^* = 35.42\%$，混合法为 $V^* = 38.81\%$。位移法由于应力后处理的数值抹平效应低估了局部危险峰值，导致材料被过度切削，陷入“过优化”陷阱；而 Hu–Zhang 混合法凭借原生高精度应力评估，准确捕捉到了危险区域，通过保留更多材料（$V^* = 38.81\%$）协同分担载荷，切实保障了结构的真实承载安全；
4. **算法收敛速度与稳定性提升**：由于底层高精度应力场提供了平滑、连续且高度可靠的伴随梯度信息，Hu–Zhang 混合法收敛更为平稳稳健，有效避免了高频震荡。这充分体现了 Hu–Zhang 混合元在驱动高度非线性局部应力约束优化时的卓越计算效率与数值鲁棒性。
