...
**状态：第一稿；正文去除空白后约 409 个字符，满足 500 字限制，待导师审阅。**

**研究计划：**0—6个月，复现并统一精确组装、精确 Matrix-Free/Krylov 及 GPU 基线，冻结二维、三维问题、局部表示接口和评价体系。6—12个月，完成候选 PIML 局部表示、结构检查、误差评价和 GPU 批量预测，形成 PIML–GPU 局部执行原型。12—18个月，在前两阶段门禁通过后，完成 PIML–Matrix-Free 全局作用、代理预条件及误差—收敛关系研究，构建三线融合数据流和可靠性机制。18—24个月，完成二维、三维结构分析与拓扑优化对照，评价精度、收敛性、完整时间、峰值内存和规模扩展，界定适用范围。

**预期成果：**形成 PIML–Matrix-Free、PIML–GPU 及三线融合三类算法原型，围绕经验证的科学问题形成并投稿相关学术论文；完成可复用软件模块，建立统一的二维、三维典型算例与性能评估体系，形成可复核的误差与性能证据及方法适用条件，为大规模拓扑优化的高效可靠求解提供方法和实现基础。

### 6. 研究基础（限 1000 字）

**状态：第二稿；已按“企业项目 Matrix-Free/GPU 直接基础—FEALPy/SOPTX 软件基础—PIML 与博士后平台基础—融合增量边界”组织，正文去除空白后约 820 个字符，满足 1000 字限制，待导师审阅。**

在企业工程计算软件研发项目中，申请人承担三维线弹性 Matrix-Free 算子及 GPU 加速相关研发工作，围绕不同单元类型与阶次，开展局部有限元算子构造、全局算子按需作用、既有求解框架接口适配、CPU/GPU 异构执行以及 Krylov 迭代求解集成，并进行数值一致性与性能评价。相关工作使申请人具备从有限元离散、Matrix-Free 算子设计到 GPU 实现和工程软件集成的系统经验，可直接支撑本项目的全局 Matrix-Free 重构、预条件迭代求解和 GPU 协同执行研究。

博士阶段，申请人依托湘潭大学数学与计算科学学院及算海团队，在 FEALPy 和 SOPTX 中开展张量化有限元、线性算子、Krylov 求解、多后端计算及 GPU 加速研究，积累了基于 NumPy、PyTorch、JAX 等后端组织数值算法和开展 CPU/GPU 一致性验证的经验。申请人以第一作者发表 SOPTX 软件论文，并作为第一完成人取得《结构拓扑优化仿真计算软件》软件著作权，为本项目的数据布局、算子接口、后端迁移和可复用软件模块开发提供了基础。

```mermaid
flowchart LR
    subgraph SOPTX["SOPTX / FEALPy 拓扑优化与多后端计算框架"]
        A1["有限元离散 & 拓扑优化抽象"] --> A2["NumPy / PyTorch / JAX / CUDA 多后端"]
        A2 --> A3["CPU/GPU 矩阵无关算子与 Krylov 求解器"]
    end
```

<center><b>图 2 自主 SOPTX/FEALPy 拓扑优化与多后端计算框架</b></center>

> **图 2 说明：** 申请人自主开发的拓扑优化与多后端计算软件平台架构（已发表 CiCP 论文并获软著授权），支撑本项目的数据布局、算子抽象与多后端迁移。

在 PIML 相关前期工作中，申请人已完成子结构静力缩聚、接口方程和细尺度响应恢复的精确验证，建立了“局部材料分布—可复用局部力学表示—局部算子—全局结构响应”的参考计算链，为后续 PIML 模型的训练、结构检查、误差评价和精确回退提供真值与接口基础。博士后阶段，项目依托郭旭院士团队及大连工业软件创新研究院在计算力学、结构拓扑优化、PIML、大规模并行计算和工业软件方面的研究积累与应用条件。

```mermaid
flowchart LR
    subgraph P1["(a) PIML 局部响应预测与精确解对比"]
        direction TB
        B1["局部材料分布快照"] --> B2["位移/应力云图与点对点误差"]
    end

    subgraph P2["(b) Matrix-Free / GPU 实测性能"]
        direction TB
        C1["Krylov 残差下降曲线"] --> C2["CPU 组装 vs GPU Matrix-Free 显存/时间对比"]
    end

    P1 --- P2
```

<center><b>图 3 PIML 局部响应恢复与 Matrix-Free/GPU 算法实测计算结果</b></center>

> **图 3 说明：** (a) 前期建立的 PIML / 子结构缩聚局部力学响应预测与精确参考解的云图及误差对比；(b) 前期开展的 Matrix-Free 算子与 GPU 加速实测残差收敛曲线及显存/时间对比。


上述基础分别覆盖 PIML 局部表示接口、Matrix-Free/Krylov 求解、GPU 异构执行和科学计算软件开发，但尚未形成 PIML–Matrix-Free–GPU 的统一融合系统。博士阶段形成的计算数学与高性能数值实现能力，与博士后平台提供的力学问题、PIML 方法和工业软件验证条件相互补充，能够支撑本项目从精确基线、分项原型到二维、三维完整拓扑优化验证的逐级实施。
