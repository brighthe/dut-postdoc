...
项目依赖保持为 `WP1 ∥ WP2 → WP3`。本页只定义融合所需的交接契约；工作包状态仍以 [[project-plan#三、工作包与依赖|项目计划]]为准。

| 阶段 | 输入 | 核心工作 | 阶段输出 | 门禁与停止条件 |
|---|---|---|---|---|
| WP1：精确 Matrix-Free/GPU 基线 | 冻结的二维／三维问题、可承受规模的精确组装参考及已验真的精确 Matrix-Free 参考 | 建立精确局部算子的 FA/EA/PA/UA、Krylov、预条件及 CPU/GPU 对照 | 统一 `update/apply/diagonal` 语义、正确性、真残差、完整 solve 时间和峰值内存 evidence | 算子、边界、真残差、响应或可重放性未通过时停止，不接入 PIML |
| WP2：PIML/GPU 局部表示 | 精确局部真值、问题契约和候选表示 | 比较多尺度形函数、缩聚刚度及其他候选，建立结构检查、分布外识别、批量预测和精确回退 | 可重放的二维／三维局部表示 provider 及结构、误差、吞吐和内存 evidence | 只达到局部 MSE、缺少结构检查或不能精确回退时停止，不接入全局 Matrix-Free |
| WP3：PIML Matrix-Free/GPU 融合 | 通过门禁的 WP1 算子／求解接口和 WP2 局部表示接口 | 研究近似局部算子、预条件 Krylov、GPU 三种数据策略及拓扑演化可靠性 | 统一结构分析与拓扑优化对照、误差传播、完整时间—内存和适用范围 evidence | 未同时满足精度、真残差、收敛和统一计时边界时不形成端到端收益结论 |
...
> 本节把三份单线 guide 与单篇证据连接为“问题需求—研究现状—交叉缺口—选题价值”。它不是可直接上传的正文；第 80 批限 1000 字版本由 [[../funding/active/china-postdoc-foundation-general-grant/80th-2026-application-draft#1. 选题依据（国内外研究现状及选题价值，限 1000 字）|申请书正文]] 唯一维护。
