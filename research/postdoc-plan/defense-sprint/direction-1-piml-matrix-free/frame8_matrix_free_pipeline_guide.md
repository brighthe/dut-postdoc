...
>
> 右侧是目前已有的验证，用的是我在 soptx 里实现的多后端无矩阵原型。第一条是 MatVec 等价，在 2D 和 3D 算例上，Matrix-Free 算出的 $MF(x)$ 和显式矩阵乘 $Kx$ 的相对误差都在 $10^{-15}$ 到 $10^{-13}$，机器精度一致；这里连单元矩阵 $K_e$ 都不显式形成，而是在积分点上直接做 $B^\top DB$ 收缩，因此至少不是“先算好单元矩阵、只是不组装全局”的 EA/EbE 路径。它究竟属于 PA 还是严格 UA/NONE，还需根据积分点 $D_e$ 是否预存进一步核实。
> 
