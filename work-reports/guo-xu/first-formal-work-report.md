...
aliases:
  ...
  - "work-reports/guo-xu/2026-07-matrix-free-progress-piml-gpu-tasks"
...
当前可追溯结果包括二维、三维 CPU 单 rank FA/EA 正确性基线，均绑定 clean revision `608cedf25038ed690f6db3be5b3f24f92329c5ec`。算例采用全 Dirichlet 制造解；EA 是该 revision 已验证的不组装全局刚度矩阵路径，FA 只作为同离散条件下的参考。
...
- 二维、三维单 rank evidence 均绑定 revision `608cedf25038ed690f6db3be5b3f24f92329c5ec`；此后分布式求解器注册、CG 初值接口和多 rank FA 防误用等路径发生过调整，当前目标 revision 尚待统一重放；
- 二维、三维 1/2-rank 一致性尚未进入统一正式 evidence，不能作为并行扩展性结果；
- PA/QA、UA/NONE、预条件和性能对照尚未完成；
- GPU 尚无基于当前统一算例的正式结果。

二维、三维 revision-scoped evidence 分别位于：
